...
describe(('testa a função getURLtoFetch'), () => {
  ...
  test(('testa caso comidas e ingrediente'), () => {
    const URL = getURLtoFetch('/comidas', 'radioIngredient', 'apple');
    expect(URL).toEqual('https://www.themealdb.com/api/json/v1/1/filter.php?i=apple');
  });
  ...
  test(('testa caso radioname '), () => {
    const URL = getURLtoFetch('/comidas', 'radioIngredient');
    expect(URL).toEqual('https://www.themealdb.com/api/json/v1/1/filter.php?i=undefined');
  });
